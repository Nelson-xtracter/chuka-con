...
// ---------- Orders: Pay on Delivery purchases awaiting fulfillment ----------
...
// ---------- Orders: mark delivered (records the sale for revenue stats) ----------
router.put("/orders/:id/deliver", requireAuth, requireRole("ADMIN"), async (req, res) => {
  const order = await prisma.order.findUnique({ where: { id: req.params.id }, include: { item: true } });
  if (!order) return res.status(404).json({ error: "Order not found" });
  if (order.status !== "PENDING") return res.status(400).json({ error: "This order was already handled" });

  const commission = Math.round(order.amount * 0.7);
  await prisma.$transaction([
    prisma.order.update({ where: { id: order.id }, data: { status: "DELIVERED" } }),
    prisma.purchase.create({
      data: { itemId: order.itemId, buyerId: order.buyerId, amount: order.amount, commission },
    }),
  ]);
...
// ---------- Orders: cancel ----------
router.put("/orders/:id/cancel", requireAuth, requireRole("ADMIN"), async (req, res) => {
