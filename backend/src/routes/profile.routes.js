...
// ---------- Exchange XP for wallet cash ----------
// Rate: KES 0.007 per XP (so 1,000 XP = KES 7). A minimum XP balance is
// required per exchange so this isn't worth doing in tiny, spammy chunks -
// once a user has reached that minimum they can cash out.
const XP_TO_KES_RATE = 0.007;
const MIN_XP_TO_EXCHANGE = 1000;

router.post("/xp/exchange", requireAuth, async (req, res) => {
  try {
    const { xpAmount } = req.body;
    const amount = parseInt(xpAmount, 10);

    if (!amount || amount <= 0) {
      return res.status(400).json({ error: "Enter a valid XP amount" });
    }
    if (amount < MIN_XP_TO_EXCHANGE) {
      return res.status(400).json({
        error: `You need at least ${MIN_XP_TO_EXCHANGE.toLocaleString()} XP to exchange for cash.`,
      });
    }
    if (amount > req.user.xp) {
      return res.status(400).json({ error: "You don't have that much XP" });
    }

    const kesAmount = Math.floor(amount * XP_TO_KES_RATE * 100) / 100; // 2dp KES

    const user = await prisma.user.update({
      where: { id: req.user.id },
      data: {
        xp: { decrement: amount },
        walletBalance: { increment: Math.round(kesAmount) },
      },
    });
    await prisma.xpEvent.create({
      data: { userId: req.user.id, amount: -amount, reason: `Exchanged for KES ${kesAmount}` },
    });

    res.json({ user: toUserDto(user), kesCredited: Math.round(kesAmount) });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: "Could not process XP exchange" });
  }
});

// ---------- Referrals: my code + how many friends joined + XP earned from it ----------
router.get("/referrals", requireAuth, async (req, res) => {
  const friendsJoined = await prisma.user.count({ where: { referredById: req.user.id } });
  const xpEvents = await prisma.xpEvent.findMany({
    where: { userId: req.user.id, reason: { contains: "Referral bonus" } },
  });
  const xpEarned = xpEvents.reduce((sum, e) => sum + e.amount, 0);

  res.json({ code: req.user.referralCode, friendsJoined, xpEarned });
});
