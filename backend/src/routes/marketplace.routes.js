...
router.post("/:id/order", requireAuth, async (req, res) => {
  ...
  if (!item.allowPayOnDelivery) {
    return res.status(400).json({ error: "This listing isn't available for order right now" });
  }

  const { phone, address, paymentMethod } = req.body;
  if (!phone) return res.status(400).json({ error: "A contact phone number is required" });
  const method = paymentMethod === "MPESA_TILL" ? "MPESA_TILL" : "CASH";
  ...
  // Live push to every admin so new orders don't need a manual refresh -
  // same "each user has a personal room" mechanism chat already uses.
  ...
  if (io) {
    ...
    admins.forEach((admin) => io.to(`user:${admin.id}`).emit("order:new", payload));
  }
